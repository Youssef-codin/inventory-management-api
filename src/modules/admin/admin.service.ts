...
export async function getAdminById(id: AdminIdInput['id']) {
    ...
    if (!admin) throw new AppError(404, 'Admin Not Found', ERROR_CODE.NOT_FOUND);
...
export async function createAdmin(data: CreateAdminInput) {
    ...
    if (exists) throw new AppError(409, 'Username is already taken', ERROR_CODE.USERNAME_TAKEN);
...
export async function updateAdmin(id: AdminIdInput['id'], data: UpdateAdminInput) {
...
export async function deleteAdmin(id: AdminIdInput['id']) {
    const exists = await findById(id);
